...
# expect -50 degrees for solar.GetAzimuth(42.364908,-71.112828,datetime.datetime(2007, 2, 18, 20, 18, 0, 0))
    ...
    altitude_rad = math.radians(GetAltitudeFast(latitude_deg, longitude_deg, utc_datetime))

    # korekcia numerickej chyby - pretecenie hodnoty pre arcsin
    value = (math.cos(declination_rad) * math.sin(hour_angle_rad) / math.cos(altitude_rad))
    if value <= -1.00:
        value = -1.00
    if value >= 1.00:
        value = 1.00
