...
def GetRadiationDirect(utc_datetime, altitude_deg):
    ...
    if(altitude_deg > 0):
        ...
        print(utc_datetime, air_mass_ratio, optical_depth)
        #return (flux * math.exp(-1 * optical_depth * air_mass_ratio))

        # znecisteny vzduch
        # return 1.1 * flux * math.pow(0.56, math.pow(air_mass_ratio, 0.715))

        return 1.1 * flux * math.pow(0.76, math.pow(air_mass_ratio, 0.618))
