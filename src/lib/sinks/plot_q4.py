...
class Plot_Q4(Component):
    ...
    def __init__(self, name, pos):
        ...
        self.maxLines = 4		# maximalny pocet zobrazenych grafov
        self.x_size = 240
        self.y_size = 180

        self.data_x = []		# datove polia
        self.data_y = [list() for _ in range(self.maxLines)]
    ...
    def drawShape(self, gc):
        ...
        for j in range(self.maxLines):
            path = QPainterPath()
            path.moveTo(0, self.y_size - 5 - j * 45)
            path.lineTo(self.x_size, self.y_size - 5 - j * 45)
            gc.drawPath(path)
        ...
        for j in range(self.maxLines):
            ...
            # iteracia po jednotlivych stopach, vyber farby stopy
                ...
                path.moveTo(
                    0, self.y_size - 5 - j * 45 - self.data_y[j][0] * 40)
                # vykreslenie dat stopy
                for i in range(1, len(self.data_x)):
                    xp = self.data_x[i] * cx
                    yp = self.y_size - 5 - j * 45 - self.data_y[j][i] * 40
                    path.lineTo(xp, yp)
    ...
    def sim(self, flag, value, time, step):
        ...
        if flag == SIM_INIT or flag == SIM_RESET: 	# re / inicializaci dat
            ...
            for q in range(self.maxLines):  # komparator a zaradenie hodnot do poli stop
                if temp[q] >= 0.5:
                    self.data_y[self.maxLines - q - 1].append(1)
                else:
                    self.data_y[self.maxLines - q - 1].append(0)
